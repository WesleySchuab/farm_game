...
var corn_hasvest_scene = preload("res://scenes/objects/plants/corn_harvest.tscn")
var tomato_hasvest_scene = preload("res://scenes/objects/plants/tomato_harvest.tscn")
...
func on_feed_the_animals()-> void:
	if in_range:
		trigger_feed_harvest("corn", corn_hasvest_scene)
		trigger_feed_harvest("tomato", tomato_hasvest_scene)
...
func trigger_feed_harvest(invetory_item: String, scene: Resource)-> void:
	...
	for index in inventory_item_count:
		...
		InventoryManager.revome_collectable(invetory_item)
...
func add_reward_scene()-> void :
	for scene in output_reward_scenes:
		...
		var reward_position: Vector2 = get_randon_position_in_circle(reward_marker.global_position, reward_output_radius)
		reward_scene.global_position = reward_position
		get_tree().root.add_child(reward_scene)
		#calculate de randow position for our award
		
func get_randon_position_in_circle(center: Vector2, raduis: int)-> Vector2:
	var angle = randf() * TAU
	var distance_from_center  = sqrt(randf()) * raduis
